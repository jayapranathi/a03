...
QUnit.test('Testing calculateArea function with several  of inputs', function (assert) {
  assert.equal(isEven(2),true, 'iseven(2) returns true');
  assert.equal(isEven(1),false, 'isEven(1)  returns false');
  assert.equal(isEven(21),false, 'isEven(21)  returns false');

  assert.throws(function () { isEven('a'); }, /The given argument is not a number/, 'Passing in a string correctly raises an Error');
